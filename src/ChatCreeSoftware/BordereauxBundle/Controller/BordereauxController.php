...
class BordereauxController extends Controller {
    ...
    public function listeProjectBordereauxAction( Request $request, Project $project ){
        ...
        foreach( $project->getFiles("") as $file ){
            $files[] = [ "filename" => $file->getFilename(), "foldername" => ""];
        }
        foreach( $folderIterator = $project->getFolders() as $folder ){
            foreach( $project->getFiles( "/" . $folder->getFilename() ) as $file ){
                $files[] = [ "filename" => $file->getFilename(), "foldername" => $folder->getFilename()];
            }
        }
