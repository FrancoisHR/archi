...
class Ligne
{
    ...
    public function getLigneDevis( $devisArray, $option ){
        $devis = [];
        $countDevis=0;
        foreach( $this->getLignesDevis() as $ligneDevis ){
            if(in_array($ligneDevis->getDevis()->getId(), $devisArray )){
        ...
        for( $n=0; $n<count($devisArray)-$countDevis; $n++ ){
            $devis[]=0;
        }
        return $devis;
    }
    
    public function totalDevis( $devisArray, $option ){
        $total = [];
        if( $this->getFilles()->count() > 0 ) {
            foreach( $this->getFilles() as $ligne ) {
                $totalFille = $ligne->totalDevis( $devisArray, $option );
            ...
            $total = $this->getLigneDevis( $devisArray, $option );
        }
        return $total;
    }
